...
def test_randn(backend):
    ...
    for dtype in dtypes[backend]["rand"]:
        tnI = tensornetwork.randn(shape, dtype=dtype, seed=seed, backend=backend)
...
def test_random_uniform(backend):
    ...
    for dtype in dtypes[backend]["rand"]:
        tnI = tensornetwork.random_uniform(
            shape, dtype=dtype, seed=seed, boundaries=boundaries, backend=backend
        )
...
def test_ones_like(backend, shape, n):
    ...
    def inner_ones_test(dtype):
        ...
        tensor = tensornetwork.ones_like(objTensor, dtype=dtype, backend=backend)
        numpyT = tensornetwork.ones_like(n, dtype=dtype, backend=backend)
...
def test_zeros_like(backend, shape, n):
    ...
    def inner_zero_test(dtype):
        ...
        tensor = tensornetwork.zeros_like(objTensor, dtype=dtype, backend=backend)
        numpyT = tensornetwork.zeros_like(n, dtype=dtype, backend=backend)
