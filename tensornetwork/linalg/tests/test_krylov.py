...
def test_matvec_cache(sparse_backend, dtype):
    ...
    A = tensornetwork.linalg.initialization.ones(
...
def test_eigsh_lanczos_raises():
    n = 2
    shape = (n, n)
    tensor = tensornetwork.linalg.initialization.ones(
        shape, backend="jax", dtype=np.float64
    )
    ...
    with pytest.raises(ValueError):
        _ = tensornetwork.linalg.krylov.eigsh_lanczos(matvec, backend=None, x0=None)
    with pytest.raises(ValueError):
        _ = tensornetwork.linalg.krylov.eigsh_lanczos(
            matvec, backend="numpy", x0=tensor
        )
    with pytest.raises(TypeError):
        _ = tensornetwork.linalg.krylov.eigsh_lanczos(
            matvec, backend="numpy", x0=tensor.array
        )
    with pytest.raises(TypeError):
        _ = tensornetwork.linalg.krylov.eigsh_lanczos(
            matvec,
...
def test_eigsh_lanczos(sparse_backend, dtype):
    ...
    x0 = tensornetwork.linalg.initialization.ones(
        (n, 1), backend=sparse_backend, dtype=dtype
    )

    def matvec(B):
        return A @ B

    result = tensornetwork.linalg.krylov.eigsh_lanczos(
        matvec, backend=A.backend, x0=x0, num_krylov_vecs=n - 1
    )
    ...
    rev, reV = result
    test_result = A.backend.eigsh_lanczos(
        array_matvec, initial_state=x0.array, num_krylov_vecs=n - 1
    )
    tev, teV = test_result
...
def test_eigsh_lanczos_with_args(sparse_backend, dtype):
    ...
    test = tensornetwork.linalg.krylov.eigsh_lanczos(
        matvec_args,
        backend=A.backend,
        x0=x0,
        num_krylov_vecs=n - 1,
...
def test_eigs_raises():
    ...
    with pytest.raises(ValueError):
        _ = tensornetwork.linalg.krylov.eigs(matvec, backend=None, x0=None)
    with pytest.raises(ValueError):
        _ = tensornetwork.linalg.krylov.eigs(matvec, backend="numpy", x0=tensor)
    with pytest.raises(TypeError):
        _ = tensornetwork.linalg.krylov.eigs(matvec, backend="numpy", x0=tensor.array)
    with pytest.raises(TypeError):
        _ = tensornetwork.linalg.krylov.eigs(
            matvec,
            backend="jax",
            x0=tensor,
            args=[
...
def test_eigs(sparse_backend, dtype):
    ...
    test_result = tensor.backend.eigs(
        test_matvec, initial_state=x0.array, num_krylov_vecs=3, numeig=1
    )
    tev, _ = test_result
...
def test_eigs_with_args(sparse_backend, dtype):
    ...
    rev, _ = result
    test_result = tensornetwork.linalg.krylov.eigs(
        test_matvec,
        x0=x0,
        num_krylov_vecs=3,
        numeig=1,
...
def test_gmres_raises():
    ...
    tensornp = tensornetwork.linalg.initialization.ones(
        shape, backend="numpy", dtype=np.float64
    )
    ...
    with pytest.raises(TypeError):
        _ = tensornetwork.linalg.krylov.gmres(
            matvec,
            tensor,
            A_args=[
                tensor.array,
...
def test_gmres(dtype, sparse_backend):
    ...
    xT, _ = tensornetwork.linalg.krylov.gmres(A_mv, b, x0=x0, num_krylov_vectors=n_kry)
...
def test_gmres_with_args(dtype, sparse_backend):
    ...
    x, _ = tensornetwork.linalg.krylov.gmres(A_mv, b, x0=x0, num_krylov_vectors=n_kry)
    xT, _ = tensornetwork.linalg.krylov.gmres(
        A_mv_test,
        b,
        x0=x0,
