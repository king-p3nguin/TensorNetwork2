...
def test_einsum_invalid_backends(dtype, backend):
    ...
    for other_backend in other_backend_names:
        ...
        for other_other_backend in backend_names:
            ...
            tensor3 = tensornetwork.zeros(
                shape, backend=other_other_backend, dtype=dtype3
            )
            with pytest.raises(ValueError):
                _ = tensornetwork.einsum(
                    "ba, bc, dc", tensor1, tensor2, tensor3, optimize=True
                )
