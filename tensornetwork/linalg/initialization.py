...
def ones_like(
    ...
    if isinstance(tensor, Tensor):
        the_tensor = initialize_tensor(
            "ones", tensor.shape, backend=tensor.backend, dtype=tensor.dtype
        )
    else:
        try:
            ...
            error = (
                "Input to zeros_like has invalid type causing "
                "error massage: \n" + str(e)
            )
            raise TypeError(error) from e
        the_tensor = initialize_tensor(
            "ones", tensor.get_shape().as_list(), backend=backend, dtype=dtype
        )
    return the_tensor
...
def zeros_like(
    ...
    if isinstance(tensor, Tensor):
        the_tensor = initialize_tensor(
            "zeros", tensor.shape, backend=tensor.backend, dtype=tensor.dtype
        )
    else:
        try:
            tensor = backend.convert_to_tensor(tensor)
            ...
            error = (
                "Input to zeros_like has invalid "
                "type causing error massage: \n" + str(e)
            )
            raise TypeError(error) from e
        the_tensor = initialize_tensor(
            "zeros", tensor.shape, backend=backend, dtype=dtype
        )
    return the_tensor
...
def randn(
    ...
    """
    the_tensor = initialize_tensor(
        "randn", shape, backend=backend, seed=seed, dtype=dtype
    )
    return the_tensor
