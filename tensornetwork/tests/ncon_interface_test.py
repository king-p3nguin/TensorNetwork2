...
def test_order_spec(backend):
    ...
    result = ncon_interface.ncon(
        [a, a], [(-1, 1), (1, -2)], con_order=[1], out_order=[-1, -2], backend=backend
    )
    np.testing.assert_allclose(result, a @ a)

    result = ncon_interface.ncon(
        [a, a], [(-1, 1), (1, -2)], con_order=[1], out_order=[-2, -1], backend=backend
    )
...
def test_node_order_spec(backend):
    ...
    result = ncon_interface.ncon(
        [node, node], [(-1, 1), (1, -2)], out_order=[-1, -2], backend=backend
    )

    np.testing.assert_allclose(result.array, a @ a)
    result = ncon_interface.ncon(
        [node, node], [(-1, 1), (1, -2)], con_order=[1], backend=backend
    )
    np.testing.assert_allclose(result.array, a @ a)
...
def test_outer_product_1(backend):
    ...
    res = ncon_interface.ncon([a, a, a, a], [(1,), (1,), (2,), (2,)], backend=backend)
...
def test_outer_product_1_mixed_labels(backend):
    ...
    res = ncon_interface.ncon(
        [a, a, a, a], [("hi",), ("hi",), ("ho",), ("ho",)], backend=backend
    )
    np.testing.assert_allclose(res, 196)
...
def test_outer_product_2(backend):
    ...
    b = np.random.rand(8)
    res = ncon_interface.ncon(
        [a, b], [(-1, -2), (-3,)], out_order=[-2, -1, -3], backend=backend
    )
...
def test_outer_product_2_mixed_labels(backend):
    ...
    b = np.random.rand(8)
    res = ncon_interface.ncon(
        [a, b], [(-1, "-hi"), ("-ho",)], out_order=["-hi", -1, "-ho"], backend=backend
    )
    exp = np.einsum("ij,k->jik", a, b)
    np.testing.assert_allclose(res, exp)
...
def test_small_matmul_mixed_labels(backend):
    ...
    a = np.random.randn(2, 2)
    b = np.random.randn(2, 2)
    res = ncon_interface.ncon([a, b], [("hi", -1), ("hi", "-ho")], backend=backend)
    np.testing.assert_allclose(res, a.transpose() @ b)
...
def test_contraction_mixed_labels(backend):
    ...
    res = ncon_interface.ncon(
        [a, a, a],
        [(-1, "rick", 2), ("rick", 2, "morty"), ("morty", -2, -3)],
        backend=backend,
    )
    res_np = a.reshape((2, 4)) @ a.reshape((4, 2)) @ a.reshape((2, 4))
    res_np = res_np.reshape((2, 2, 2))
...
def test_node_contraction(backend):
    ...
    res_np = tensor.reshape((2, 4)) @ tensor.reshape((4, 2)) @ tensor.reshape((2, 4))
    res_np = res_np.reshape((2, 2, 2))
...
def test_get_cont_out_labels_1():
    ...
    int_cont_labels, str_cont_labels, int_out_labels, str_out_labels = (
        _get_cont_out_labels(network_structure)
    )
...
def test_canonicalize_network_structure():
    network_structure = [[-3, 10, 15, "-5"], [-5, -23, 8, "66", "60"], [3, 4, 5, "6"]]
...
def test_batched_outer_product(backend):
    ...
    res = ncon_interface.ncon(
        [a, b], [(-1, -3), (-2, -3)], out_order=[-2, -1, -3], backend=backend
    )
    exp = np.einsum("ik,jk->jik", a, b)
    np.testing.assert_allclose(res, exp)

    res = ncon_interface.ncon(
        [a, b], [(-1, -3), (-2, -3)], out_order=[-2, -3, -1], backend=backend
    )
...
def test_batched_matmul_1(backend):
    ...
    res = ncon_interface.ncon(
        [a, b], [(-1, 1, -3), (1, -3, -2)], out_order=[-2, -1, -3], backend=backend
    )
    exp = np.einsum("ijk,jkm->mik", a, b)
    np.testing.assert_allclose(res, exp)

    res = ncon_interface.ncon(
        [a, b], [(-1, 1, -3), (1, -3, -2)], out_order=[-3, -2, -1], backend=backend
    )
...
def test_batched_matmul_2(backend):
    ...
    res = ncon_interface.ncon(
        [a, b, c], [(-1, 1, 2, -2), (1, -3, -2, 3), (-2, 3, 2)], backend=backend
    )
...
def test_batched_matmul_3(backend):
    ...
    c = np.random.randn(batchsize, 5, 4)
    res = ncon_interface.ncon(
        [a, b, c], [(-1, 1, 2, 4), (1, -2, 4, 3), (4, 3, 2)], backend=backend
    )
...
def run_tests(a, b, c, backend):
    ...
    with pytest.raises(
        ...
        ncon_interface.ncon(
            [a, a], [("megan!", "henry@"), ("henry@", "megan!")], backend=backend
        )

    with pytest.raises(
        ValueError,
        match="number of tensors does not " "match the number of network connections.",
    ):
        ncon_interface.ncon([a, a], [(1, 2), (2, 1), (1, 2)], backend=backend)
    with pytest.raises(
        ValueError,
        match="number of indices does not match " "number of labels on tensor 0.",
    ):
        ncon_interface.ncon([a, a], [(1,), (1, 2)], backend=backend)

    with pytest.raises(
        ValueError,
        match="only nonzero values are allowed to " "specify network structure.",
    ):
    ...
    with pytest.raises(
        ...
        ncon_interface.ncon(
            [a, a], [(1, 2), (2, 1)], con_order=[-1, 2], backend=backend
        )
    with pytest.raises(
        ValueError,
        ...
        ncon_interface.ncon(
            [a, a], [(1, 2), (2, 1)], con_order=["-hi", 2], backend=backend
        )

    with pytest.raises(
        ValueError, match=r"labels \['hi', 1\] appear more than once in `con_order`."
    ):
        ncon_interface.ncon(
            [a, a], [(1, 2), (2, 1)], con_order=["hi", "hi", 1, 1], backend=backend
        )
    with pytest.raises(
        ValueError,
        ...
        ncon_interface.ncon(
            [a, a], [(1, 2), (2, 1)], con_order=[3, 4, 5], backend=backend
        )
    ...
    with pytest.raises(
        ...
        ncon_interface.ncon([a, a], [(1, 2), (2, 1)], con_order=[3, 4], backend=backend)
    ...
    with pytest.raises(
        ...
        ncon_interface.ncon(
            [a, a], [("-hi", 1), (1, -2)], out_order=["hi", -2], backend=backend
        )

    with pytest.raises(
        ValueError, match=r"labels \['-hi', -1\] appear more than once in `out_order`."
    ):
        ncon_interface.ncon(
            [a, a], [(1, 2), (2, 1)], out_order=["-hi", "-hi", -1, -1], backend=backend
        )
    ...
    with pytest.raises(
        ...
        ncon_interface.ncon(
            [a, a], [(-1, 2), (2, -2)], out_order=[-1, -2, -3], backend=backend
        )
    ...
    with pytest.raises(
        ...
        ncon_interface.ncon(
            [a, a], [(-1, 2), (2, -2)], out_order=[-3, -4], backend=backend
        )

    with pytest.raises(
        ValueError, match=r"tensor dimensions for labels \[2, 4\] " r"are mismatching"
    ):
...
def test_node_invalid_network(backend):
    ...
    c = np.ones((2, 4, 4, 3))
    run_tests(
        Tensor(a, backend=backend),
        Tensor(b, backend=backend),
        Tensor(c, backend=backend),
        backend,
    )
