...
def get_zeros(shape, dtype=np.float64):
    ...
    flows = list(np.full(R, fill_value=False, dtype=bool))
...
def test_network_copy_reordered(dtype, num_charges):
    a = tn.Node(
        get_random_symmetric(
            (30, 30, 30), [False, False, False], num_charges, dtype=dtype
        ),
        backend="symmetric",
    )
    b = tn.Node(
        get_random_symmetric(
            (30, 30, 30), [False, True, False], num_charges, dtype=dtype
        ),
        backend="symmetric",
    )
    c = tn.Node(
        get_random_symmetric(
            (30, 30, 30), [True, False, True], num_charges, dtype=dtype
        ),
        backend="symmetric",
    )
...
def test_double_trace(dtype, num_charges):
    a = tn.Node(
        get_random_symmetric(
            (20, 20, 20, 20), [True, False, True, False], num_charges, dtype=dtype
        ),
        backend="symmetric",
    )
...
def test_real_physics(dtype, num_charges):
    ...
    t1 = get_random_symmetric(
        (20, 20, 20, 20), [False, False, False, False], num_charges, dtype=dtype
    )
    t2 = get_random_symmetric(
        (20, 20, 20), [True, False, True], num_charges, dtype=dtype
    )
    t3 = get_random_symmetric(
        (20, 20, 20), [True, True, False], num_charges, dtype=dtype
    )
...
def test_node2_contract_trace(dtype, num_charges):
    a = tn.Node(
        get_random_symmetric(
            (20, 20, 20), [False, True, False], num_charges, dtype=dtype
        ),
        backend="symmetric",
    )
    b = tn.Node(
...
def test_flatten_consistent_result(dtype, num_charges):
    a_val = get_random_symmetric(
        (10, 10, 10, 10), [False] * 4, num_charges, dtype=dtype
    )
...
def test_flatten_trace_consistent_tensor(dtype, num_charges):
    a_val = get_random_symmetric(
        (5, 5, 5, 5, 5), [False, False, True, True, True], num_charges, dtype=dtype
    )
    ...
    a_final = np.reshape(np.transpose(a_val.todense(), (1, 2, 0, 3, 4)), (5, 25, 25))
...
def test_contract_between_trace_edges(dtype, num_charges):
    a_val = get_random_symmetric((50, 50), [False, True], num_charges, dtype=dtype)
...
def test_at_operator(dtype, num_charges):
    ...
    np.testing.assert_allclose(
        c.tensor.todense(), a.tensor.todense() @ b.tensor.todense()
    )
