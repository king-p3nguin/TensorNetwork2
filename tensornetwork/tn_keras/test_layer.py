...
def test_decomp_num_parameters(dummy_data):
    ...
    expected_num_parameters = (
        (data.shape[1] * decomp_size) + (decomp_size * output_dim) + output_dim
    )
...
def test_mpo_num_parameters(dummy_data):
    ...
    in_leg_dim = math.ceil(data.shape[1] ** (1.0 / num_nodes))
    out_leg_dim = math.ceil(output_dim ** (1.0 / num_nodes))

    # num_params = num_edge_node_params + num_middle_node_params + bias_params
    expected_num_parameters = (
        (2 * in_leg_dim * bond_dim * out_leg_dim)
        + ((num_nodes - 2) * in_leg_dim * bond_dim * bond_dim * out_leg_dim)
        + output_dim
    )
...
def test_condenser_num_parameters(dummy_data):
    ...
    expected_num_parameters = (
        num_nodes * output_dim * output_dim * exp_base
    ) + output_dim
...
def test_expander_num_parameters(dummy_data):
    ...
    # num_params = (num_nodes * num_node_params) + num_bias_params
    expected_num_parameters = (
        num_nodes * data.shape[-1] * data.shape[-1] * exp_base
    ) + output_dim
...
def test_entangler_asymmetric_num_parameters_output_shape(
    ...
    expected_num_parameters = (
        (num_levels - 1) * (num_legs - 1) * (primary**4)
        + (num_legs - 2) * primary**3 * secondary
