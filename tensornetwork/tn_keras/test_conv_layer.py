...
def test_num_parameters(dummy_data, make_model):
    ...
    in_dim = math.ceil(data.shape[-1] ** (1.0 / l.num_nodes))
    out_dim = math.ceil(l.filters ** (1.0 / l.num_nodes))
    exp_num_parameters = (
        (l.num_nodes - 2) * (l.bond_dim * 2 * in_dim * out_dim)
...
def test_model_save(dummy_data, make_model, tmp_path):
    ...
    for save_path in [tmp_path / "test_model.keras", tmp_path / "test_model.h5"]:
        ...
        np.testing.assert_almost_equal(model.predict(data), loaded_model.predict(data))
