    ...
    for i in range(1, N):
        for j in range(i):
            ...
            if num_kept > 0:
                ...
                single_cost = np.reshape(
                    comb_dims - log_adj[j, i, new_pos], [1, num_kept]
                )
                if costs.size == 0:
                    new_costs = single_cost
                else:
                    prev_cost = costs[0, new_pos]
                    new_costs = prev_cost + np.log10(
                        1 + 10 ** (single_cost - prev_cost)
                    )
            ...
            if num_kept > 0:
                ...
                dims = np.sum(log_adj[:, :, new_pos], axis=0).reshape(N, num_kept)
                comb_dims = dims[j, :] + dims[i, :]
                final_dims = np.reshape(
                    comb_dims - 2 * log_adj[j, i, new_pos], [1, num_kept]
                )
