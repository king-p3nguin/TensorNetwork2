...
def test_BaseCharge_unique():
    ...
    expected = unique(q, return_index=True, return_inverse=True, return_counts=True)
...
@pytest.mark.parametrize(
    "chargetype, B0, B1",
    [(U1Charge, -5, 5), (Z2Charge, 0, 1), (ZNCharge(3), 0, 2), (ZNCharge(6), 0, 5)],
)
...
@pytest.mark.parametrize(
    "chargetype, B0, B1,sign", [(U1Charge, -5, 5, -1), (Z2Charge, 0, 1, 1)]
)
...
def get_charges(B0, B1, D, num_charges):
    return [
        np.random.randint(B0, B1 + 1, D).astype(np.int16) for _ in range(num_charges)
    ]


def fuse_many_charges(num_charges, num_charge_types, seed, D, B, use_flows=False):
    np.random.seed(seed)
    if use_flows:
    ...
    np_flows[flows] = -1
    charges = [
        get_charges(-B // 2, B // 2, D, num_charge_types) for _ in range(num_charges)
    ]
    fused = [
        fuse_ndarrays([charges[n][m] * np_flows[n] for n in range(num_charges)])
    ...
    target_charges = [
        U1Charge(np.array([np_target_charges[n]])) for n in range(num_charge_types)
    ]
...
@pytest.mark.parametrize(
    "num_charges, num_charge_types, D, B",
    [
        (1, 1, 0, 5),
        (2, 1, 1000, 6),
        (2, 2, 1000, 6),
...
@pytest.mark.parametrize(
    "chargetype, B0, B1", [(U1Charge, -5, 5), (Z2Charge, 0, 1), (ZNCharge(3), 0, 2)]
)
...
def test_Charge_mul_raises():
    ...
    q = U1Charge(C)
    with pytest.raises(ValueError, match="can only multiply by `True` or `False`"):
