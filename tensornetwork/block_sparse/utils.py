...
def _random(
    size: int, dtype: Type[np.number] = np.float64, boundaries: Tuple = (0, 1)
) -> np.ndarray:
    """
    Initialize a 1d np.ndarray of length `size` of dtype `dtype`
    ...
    if (np.dtype(dtype) is np.dtype(np.complex128)) or (
        ...
        data += 1j * np.random.uniform(boundaries[0], boundaries[1], size).astype(dtype)
    return data
...
def fuse_stride_arrays(
    dims: Union[List[int], np.ndarray], strides: Union[List[int], np.ndarray]
) -> np.ndarray:
    """
...
def fuse_degeneracies(
    degen1: Union[List, np.ndarray], degen2: Union[List, np.ndarray]
) -> np.ndarray:
    """
    ...
    """
    return np.reshape(
        np.array(degen1)[:, None] * np.array(degen2)[None, :], len(degen1) * len(degen2)
    )
...
def _find_best_partition(dims: Union[List[int], np.ndarray]) -> int:
    ...
    """
    if len(dims) == 1:
        raise ValueError("expecting dims with a length of at least 2, got len(dims) =1")
    diffs = [
        np.abs(np.prod(dims[0:n]) - np.prod(dims[n::])) for n in range(1, len(dims))
    ]
...
def collapse(array: np.ndarray) -> np.ndarray:
    ...
    if array.shape[1] in (1, 2, 4, 8):
        ...
        tmparray = np.squeeze(
            np.concatenate(
                [array, np.zeros((array.shape[0], width), dtype=array.dtype)], axis=1
            ).view(newdtype)
        )
...
def intersect(
    A: np.ndarray, B: np.ndarray, axis=0, assume_unique=False, return_indices=False
) -> Any:
    """
    ...
    if A.dtype != B.dtype:
        raise ValueError(
            f"array dtypes must macht to intersect, "
    ...
    if axis == 1:
        ...
        out = intersect(
            A.T, B.T, axis=0, assume_unique=assume_unique, return_indices=return_indices
        )
        if return_indices:
    ...
    if collapsed_A.ndim > 1:
        ...
        return _intersect_ndarray(
            collapsed_A, collapsed_B, axis, assume_unique, return_indices
        )
    if collapsed_A.dtype in (np.int8, np.int16) and collapsed_B.dtype in (
        np.int8,
        np.int16,
    ):
        ...
        C, A_locs, B_locs = np.intersect1d(
            collapsed_A, collapsed_B, assume_unique=assume_unique, return_indices=True
        )
...
def _intersect_ndarray(
    ...
    raise NotImplementedError(
        "_intersect_ndarray is only implemented for 1d or 2d arrays"
    )
