...
def test_create_diag(dtype, num_charges, flow):
    ...
    for n, block in enumerate(sparse_blocks):
        ...
        np.testing.assert_allclose(
            arr.data[np.squeeze((index._charges[0] * flow) == charges[n])], block_diag
        )
...
def test_diag_raises():
    ...
    indices = [
        Index(
            BaseCharge(np.random.randint(-2, 3, (Ds[n], 1)), charge_types=[U1Charge]),
            False,
        )
        for n in range(rank)
    ]
...
@pytest.mark.parametrize(
    "Ds, R1",
    [([20, 21], 1), ([18, 19, 20], 2), ([18, 19, 20], 1), ([0, 10], 1), ([10, 0], 1)],
)
@pytest.mark.parametrize("num_charges", [1, 2, 3])
...
def test_qr_r_mode():
    ...
    np.testing.assert_allclose(np.abs(np.diag(R.todense())), np.abs(np.diag(R_np)))
...
@pytest.mark.parametrize(
    "Ds, R1",
    [([20, 21], 1), ([18, 19, 20], 2), ([18, 19, 20], 1), ([10, 0], 1), ([0, 10], 1)],
)
@pytest.mark.parametrize("num_charges", [1, 2, 3])
...
def test_inv(dtype, num_charges):
    ...
    flows = [True, False]
    A = BlockSparseTensor.random(
        [Index(charge, flows[n]) for n in range(R)], (-0.5, 0.5), dtype=dtype
    )
    invA = inv(A)
    left_eye = invA @ A
...
def test_inv_raises():
    ...
    A = BlockSparseTensor.random([Index(charge, False) for n in range(R)], (-0.5, 0.5))
    with pytest.raises(ValueError):
        inv(A)
...
def test_eig_prod(dtype, Ds, num_charges):
    ...
    A_ = V @ diag(E) @ inv(V)
    np.testing.assert_allclose(
        A.contiguous(inplace=True).data, A_.data
...
def test_eye(dtype, num_charges, D):
    ...
    blocks, _, shapes = _find_diagonal_sparse_blocks(A.flat_charges, A.flat_flows, 1)
    for n, block in enumerate(blocks):
...
def test_trace_matrix(dtype, num_charges, D):
    ...
    flows = [True, False]
    matrix = BlockSparseTensor.random(
        [Index(charge, flows[n]) for n in range(R)], dtype=dtype
    )
    res = trace(matrix)
...
def test_trace_raises(num_charges):
    ...
    indices = [Index(charge1, False), Index(charge2, True), Index(charge1, False)]
