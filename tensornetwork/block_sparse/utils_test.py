...
def test_collapse(N, dtype, resdtype):
    ...
    if N in (1, 2, 4):
        ...
        expected = np.squeeze(
            np.concatenate([a, np.zeros((D, 8 - N), dtype=dtype)], axis=1).view(
                resdtype
            )
        )
...
def test_unique(N, label_dtype, D, return_index, return_inverse, return_counts):
    ...
    def test_array(act, exp):
        if N != 3:
            ...
            t1 = np.squeeze(
                np.concatenate(
                    [exp, np.zeros((exp.shape[0], 1), dtype=np.int16)], axis=1
                ).view(get_dtype(2 * N))
            )

            t2 = np.squeeze(
                np.concatenate(
                    [act, np.zeros((act.shape[0], 1), dtype=np.int16)], axis=1
                ).view(get_dtype(2 * N))
            )
