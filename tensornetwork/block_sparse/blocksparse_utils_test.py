...
def test_flat_meta_data():
    ...
    expected_charges = [i1._charges[0], i1._charges[1], i2._charges[0], i2._charges[1]]
...
def test_compute_sparse_lookup_non_ordered(flow):
    ...
    inds = np.nonzero(np.isin((np_flow * unique_charges)[charge_labels], np_targets))[0]
...
def test_compute_num_nonzero(num_charges):
    ...
    flows = [False, True, False]
    np_flows = [1, -1, 1]
    fused = fuse_many_ndarray_charges(
        [qs[n] * np_flows[n] for n in range(3)], [U1Charge] * num_charges
    )
    ...
    nz2 = len(
        np.nonzero(
            np.logical_and.reduce(
                fused == np.zeros((1, num_charges), dtype=np.int16), axis=1
            )
        )[0]
    )
    assert nz1 == nz2
...
def test_reduce_charges():
    ...
    dense_positions = reduce_charges(
        [U1Charge(left_charges), U1Charge(right_charges)],
        [False, False],
        target_charge,
    ...
    np.testing.assert_allclose(
        dense_positions[1], np.nonzero(fused_charges == target_charge[0, 0])[0]
    )


def test_reduce_charges_2():
    left_charges = np.asarray([[-2, 0, 1, 0, 0], [-3, 0, 2, 1, 0]]).astype(np.int16).T
    right_charges = np.asarray([[-1, 0, 2, 1], [-2, 2, 7, 0]]).astype(np.int16).T
    target_charge = np.zeros((1, 2), dtype=np.int16)
    fused_charges = fuse_ndarray_charges(
        left_charges, right_charges, [U1Charge, U1Charge]
    )
    dense_positions = reduce_charges(
        [
    ...
    np.testing.assert_allclose(
        dense_positions[1],
        np.nonzero(np.logical_and.reduce(fused_charges == target_charge, axis=1))[0],
    )
...
def test_reduce_charges_non_trivial(num_charges):
    ...
    fused_charges = fuse_ndarray_charges(left_charges, right_charges, charge_types)
    ...
    assert np.all(
        np.isin(np.squeeze(dense_positions[0].charges), np.squeeze(target_charge))
    )
    tmp = []
    ...
    for n in range(target_charge.shape[0]):
        # pylint: disable=no-member
        tmp.append(
            np.logical_and.reduce(fused_charges == target_charge[n, :][None, :], axis=1)
        )
    # pylint: disable=no-member
...
def test_find_diagonal_sparse_blocks(num_legs, num_charges):
    ...
    right_charges = np.stack(
        [
            fuse_ndarrays([np_charges[n][:, c] for n in range(num_legs // 2, num_legs)])
            for c in range(num_charges)
        ],
        axis=1,
    )
    # pylint: disable=no-member
    nz = np.nonzero(np.logical_and.reduce(fused == np.zeros((1, num_charges)), axis=1))[
        0
    ]
    ...
    for n in range(unique_left.shape[0]):
        ...
        # pylint: disable=no-member
        blocks.append(
            linear_locs[np.nonzero(np.logical_and.reduce(left == ul, axis=1))[0]]
        )
...
def test_find_transposed_diagonal_sparse_blocks(num_charges, order, D):
    ...
    transposed_linear_positions = fuse_stride_arrays(dims, [strides[o] for o in order])
    left_charges = np.stack(
        [
            fuse_ndarrays([np_charges[order[n]][:, c] for n in range(num_legs // 2)])
            for c in range(num_charges)
        ],
        axis=1,
    )
    ...
    # pylint: disable=no-member
    tr_mask = np.logical_and.reduce(tr_fused == np.zeros((1, num_charges)), axis=1)
    ...
    for n in range(unique_left.shape[0]):
        ...
        blocks.append(
            dense_to_sparse[
                tr_linear_locs[np.nonzero(np.logical_and.reduce(left == ul, axis=1))[0]]
            ]
        )

    charges = [BaseCharge(c, charge_types=[U1Charge] * num_charges) for c in np_charges]
