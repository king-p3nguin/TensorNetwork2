...
class FiniteMPS(BaseMPS):
    ...
    def left_envs(self, sites: Sequence[int]) -> dict:
        ...
        if not np.all(sites <= len(self)):
            raise ValueError(
                "all elements of `sites` have to be <= N = {}".format(len(self))
            )
        ...
        for site in left_sites:
            left_envs[site] = Node(
                self.backend.eye(
                    N=self.backend.sparse_shape(self.backend.conj(self.tensors[site]))[
                        0
                    ],
                    dtype=self.dtype,
                ),
        ...
        if n2 > center_position:
            ...
            left_env = contract_between(
                nodes[center_position], conj_nodes[center_position]
            )
