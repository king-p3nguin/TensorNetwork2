...
class BaseMPS:
    ...
    def __init__(
        ...
        def svd(
            tensor, pivot_axis=2, max_singular_values=None, max_truncation_error=None
        ):
        ...
        if site > self.center_position:
            ...
            for n in range(self.center_position, site):
                ...
                if not use_svd:
                    ...
                    isometry, S, V, _ = self.svd(
                        self.tensors[n], 2, D, max_truncation_err
                    )
                    rest = ncon.ncon(
        ...
        # shift center_position to the left using RQ or SV decomposition
            ...
            for n in reversed(range(site + 1, self.center_position + 1)):
                ...
                if not use_svd:
                    ...
                    U, S, isometry, _ = self.svd(
                        self.tensors[n], 1, D, max_truncation_err
                    )
                    rest = ncon.ncon(
    ...
    def bond_dimension(self, bond) -> list:
        ...
        if bond > len(self):
            raise IndexError(
                f"bond {bond} out of bounds for" f" an MPS of length {len(self)}"
            )
        if bond < len(self):
    ...
    def apply_one_site_gate(self, gate: Tensor, site: int) -> None:
        ...
        if len(gate.shape) != 2:
            raise ValueError(
                "rank of gate is {} but has to be 2".format(len(gate.shape))
            )
    ...
    def check_orthonormality(self, which: str, site: int) -> Tensor:
        ...
        return self.backend.sqrt(
            ncon.ncon(
                [tmp, self.backend.conj(tmp)], [[1, 2], [1, 2]], backend=self.backend
            )
        )
