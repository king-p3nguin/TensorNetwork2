...
def test_canonical_finite_mps(backend_dtype_values):
    ...
    mps = FiniteMPS(tensors, center_position=N // 2, backend=backend, canonicalize=True)
...
def test_left_envs_all_sites_non_0_center_position(backend_dtype_values):
    ...
    expected = backend.convert_to_tensor(np.array([[1, 0, 0], [0, 0, 0], [0, 0, 0]]))
    np.testing.assert_array_almost_equal(envs[0], 1.0)
    np.testing.assert_array_almost_equal(envs[3], expected)
...
def test_right_envs_all_sites(backend_dtype_values):
    ...
    np.testing.assert_array_almost_equal(envs[-1], 1.0)
    np.testing.assert_array_almost_equal(envs[2], np.eye(3))
...
def test_check_canonical_raises(backend):
    ...
    with pytest.raises(
        ValueError,
        match="FiniteMPS.center_positions is `None`. " "Cannot check canonical form.",
    ):
        mps.check_canonical()
