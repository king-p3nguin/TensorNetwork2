...
def get_XXZ_Hamiltonian(N, Jx, Jy, Jz):
    ...
    H = np.zeros((2**N, 2**N))
    for n in range(N - 1):
        H += Jx * Sx[n] @ Sx[n + 1] - Jy * Sy[n] @ Sy[n + 1] + Jz * Sz[n] @ Sz[n + 1]
    return H
...
def test_BaseDMRG_raises_2():
    ...
    mpo = FiniteXXZ(Jz, Jxy, Bz, dtype=dtype, backend=backend)
    tensors = (
        [np.random.randn(1, 2, D)]
        + [np.random.randn(D, 2, D) for _ in range(N - 2)]
        + [np.random.randn(D, 2, 1)]
    )
    mps = FiniteMPS(tensors, center_position=None, canonicalize=False, backend=backend)
    with pytest.raises(
        ValueError, match="Found mps in non-canonical form. Please canonicalize mps."
    ):
        BaseDMRG(
            mps,
            mpo,
...
def test_compute_envs(backend_dtype_values):
    ...
    np.testing.assert_allclose([0, 1, 2, 3, 4, 5], sorted(list(dmrg.left_envs.keys())))
    np.testing.assert_allclose([5, 6, 7, 8, 9], sorted(list(dmrg.right_envs.keys())))
...
def test_finite_DMRG_init(backend_dtype_values, N):
    ...
    one_site_energy_no_sweeps = dmrg.run_one_site(num_sweeps=0, num_krylov_vecs=10)
    np.testing.assert_allclose(one_site_energy_no_sweeps, one_site_energy)
    # test two-site DMRG
    mps = FiniteMPS.random([2] * N, [D] * (N - 1), dtype=dtype, backend=backend)
    dmrg = FiniteDMRG(mps, mpo)
    two_site_energy = dmrg.run_two_site(
        max_bond_dim=D, num_sweeps=4, num_krylov_vecs=10
    )
    np.testing.assert_allclose(two_site_energy, eta[0])
    two_site_energy_no_sweeps = dmrg.run_two_site(
        max_bond_dim=D, num_sweeps=0, num_krylov_vecs=10
    )
