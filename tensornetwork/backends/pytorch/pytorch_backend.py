...
class PyTorchBackend(abstract_backend.AbstractBackend):
    ...
    def eye(
        self, N: int, dtype: Optional[Any] = None, M: Optional[int] = None
    ) -> Tensor:
    ...
    def zeros(self, shape: Tuple[int, ...], dtype: Optional[Any] = None) -> Tensor:
        dtype = dtype if dtype is not None else torchlib.float64
    ...
    def eigsh_lanczos(
        ...
        if not isinstance(initial_state, torchlib.Tensor):
            raise TypeError(
                "Expected a `torch.Tensor`. Got {}".format(type(initial_state))
            )
        ...
        for it in range(num_krylov_vecs):
            ...
            if reorthogonalize:
                for v in krylov_vecs:
                    vector_n -= (
                        v.contiguous().view(-1).dot(vector_n.contiguous().view(-1))
                    ) * torchlib.reshape(v, vector_n.shape)
            krylov_vecs.append(vector_n)
            A_vector_n = A(vector_n, *args)
            diag_elements.append(
                vector_n.contiguous().view(-1).dot(A_vector_n.contiguous().view(-1))
            )

            if ((it > 0) and (it % ndiag) == 0) and (len(diag_elements) >= numeig):
                # diagonalize the effective Hamiltonian
                A_tridiag = (
                    torchlib.diag(torchlib.tensor(diag_elements))
    ...
    def inv(self, matrix: Tensor) -> Tensor:
        if len(matrix.shape) > 2:
            raise ValueError(
                "input to pytorch backend method `inv` has shape {}. Only matrices are supported.".format(
                    matrix.shape
                )
            )
    ...
    def broadcast_right_multiplication(
        ...
        if len(tensor2.shape) != 1:
            raise ValueError(
                "only order-1 tensors are allowed for `tensor2`, found `tensor2.shape = {}`".format(
                    tensor2.shape
                )
            )
    ...
    def trace(
        ...
        if offset != 0:
            errstr = (
                f"offset = {offset} must be 0 (the default)" f"with PyTorch backend."
            )
        ...
        if N > 25:
            raise ValueError(
                f"Currently only tensors with ndim <= 25 can be traced"
