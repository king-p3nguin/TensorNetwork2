...
def svd(
    ...
    if (max_singular_values is not None) and (max_singular_values >= orig_num_singvals):
    ...
    if (max_truncation_error is not None) or (max_singular_values is not None):
        ...
        if max_truncation_error is not None:
            ...
            kept_inds_mask = (
                np.sqrt(np.cumsum(np.square(extended_flat_singvals[inds])))
    ...
    Sdisc = ChargeArray(
        all_discarded_singvals, [left_discarded_singval_charge], [False]
    )
...
def rq(bt, tensor: BlockSparseTensor, pivot_axis: int) -> Tuple[Tensor, Tensor]:
    ...
    r, q = bt.conj(bt.transpose(r, (1, 0))), bt.conj(
        bt.transpose(q, (1, 0))
    )  # M=r*q at this point
