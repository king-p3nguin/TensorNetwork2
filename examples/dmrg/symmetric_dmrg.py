...
def blocksparse_XXZ_mpo(
    Jz: np.ndarray, Jxy: np.ndarray, Bz: np.ndarray, dtype: Type[np.number] = np.float64
) -> tn.FiniteMPO:
    """
    ...
    mpotensors = (
        [tn.BlockSparseTensor.fromdense([ileft, i2, i3, i4], dense_mpo[0])]
        + [
...
def blocksparse_halffilled_spin_mps(
    N: int, D: int, B: int = 5, dtype: Type[np.number] = np.float64
):
    """
...
def initialize_spin_mps(N: int, D: int, dtype: Type[np.number], backend: str):
    """
...
if __name__ == "__main__":
    ...
    for be in backends:
        ...
        energies[be] = run_twosite_dmrg(
            num_sites, bond_dim, datatype, jz, jxy, bz, num_sweeps=n_sweeps, backend=be
        )
